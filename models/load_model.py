...
def load_model(modelName, from_json = False, FULLCONV = True, training=False):
    '''
    ...
    if(from_json):
        ...
        if(FULLCONV):
            model = build_model(learning_rate=0.0002, weightsFile=weightsFile, inputSize=None, training=training)
        else:
            model = build_model(learning_rate=0.0002, weightsFile=weightsFile, inputSize=inputSize, training=training)
