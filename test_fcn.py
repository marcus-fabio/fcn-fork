import numpy as np
from prediction import run_prediction, predict_frameWise
import load as ld
import mir_eval
import os
...
def test_fragment():
    evaluate(filename='allemande_second_fragment_preston',
             confidence_levels=np.linspace(0, 0.9, 10), force_run=True)


def test_all():
    evaluate(confidence_levels=np.linspace(0, 0.9, 10))
...
def test_print_mir_fragment_results():
    save_plots('allemande_second_fragment_preston')
...
def evaluate(filename="all",
    ...
    for file in fragments:
        ...
        for conf_level in confidence_levels:
            ...
            if "fragment" in file:
                _, est_freq_crepe, confidence_crepe = get_fcn_estimation(
                    file, output="./crepe_results/", delimiter=',', force_run=force_run)

                rf = ref_freq[np.argmax(confidence_crepe)]
...
def get_fcn_estimation(file, output="./fcn_results/", delimiter="   ", force_run=False):
    ...
    if os.path.exists(fcn_results) is False or force_run:
        ...
        run_prediction([filepath], output=output, modelTag="1953")
